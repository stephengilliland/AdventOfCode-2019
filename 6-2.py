...
san = []
you = []
#inp = ["COM","B","B","C","C","D","D","E","E","F","B","G","G","H","D","I","E","J","J","K","K","L","K","YOU","I","SAN"]

for x in range(1, len(inp), 2):

    if inp[x] == "SAN" and inp[x] != "COM":
        curr = inp[x]
        while not done:
            
            for y in range(1, len(inp), 2):

                if inp[y] == curr and not done and not done2:
                    if inp[y-1] == "COM":
                        done = True
                        san.append(inp[y])
                        orbs += 1
...
for x in range(len(san)):
    currSAN = san[x]
    for y in range(len(you)):
        currYOU = you[y]
        if currYOU == currSAN and not first:
            interX = x
            interY = y
            first = True
